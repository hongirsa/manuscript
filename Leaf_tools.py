...
def Leaf_tools(mapBounds, year):
    ...
    input_collection = ee.ImageCollection(colName) \
        .filterBounds(mapBounds).filter(ee.Filter.calendarRange(year_start, year_end, 'year')).filter(
        ee.Filter.calendarRange(month_start, month_end, 'month')).filterMetadata(colOptions["Cloudcover"], 'less_than',
                                                                                 maxCloudcover) \
        .limit(5000) \
